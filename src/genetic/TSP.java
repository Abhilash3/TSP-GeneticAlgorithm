...
public class TSP {
    ...
    public ArrayList<Integer> simulate() {
        ...
        for (City city : coordinates) {
            map.drawPoint(city.getX(), city.getY());
        }
        ...
        map.drawLine(coordinates.get(bestPath.get(i)).getX(), coordinates
                    .get(bestPath.get(i)).getY());
        }
        ...
        textField.setText(String.format("%s: %6s %s", "Generation", 1,
                "         ")
                + String.format("%s: %20s", "Best Result",
        ...
        for (int i = 1; i < Generations; i++) {
            ...
            if (fitness.generateFitness(bestPath) > fitness
                    .generateFitness(generation.get(0))) {
                bestPath = generation.get(0);

                map.clearLines();
                for (int j = 0; j < CityNumber; j++) {
                    map.drawLine(coordinates.get(bestPath.get(j)).getX(),
                            coordinates.get(bestPath.get(j)).getY());
                }
            }

            textField.setText(String.format("%s: %6s %s", "Generation",
                    (i + 1), "         ")
                    + String.format("%s: %20s", "Best Result",
